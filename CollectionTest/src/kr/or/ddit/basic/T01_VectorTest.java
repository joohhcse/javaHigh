...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		// Vector는 addElement() 메서드를 이용하여 추가할 수 있는데
...
//		remove(index) -> 벡터의 index번째 자료를 삭제한다. 자료가 삭제되면 index번째 다음번째 이후의 데이터들이 앞으로 자동으로 당겨저서 채워진다.
		
//		remove(삭제할데이터) -> '삭제할 데이터'를 찾아 삭제한다
//							만약 '삭제할데이터'가 여러개이면 앞에서부터 삭제함.
//							삭제할 데이터가 '정수형'이거나 'char'형 일 경우는
...
//		Vector<String> v2 = new Vector<String>();	//이렇게 써도 된다
		Vector<String> v2 = new Vector<>();			//String 만 저장가능
		
//		Vector<Integer> v3 = new Vector<Integer>();	//이렇게 써도 된다
		Vector<Integer> v3 = new Vector<>();		//Integer 만 저장가능
...
/*
  		향상된 for 문
		형식) 
		for (자료형명 변수명 : 배열변수나 Collection 계열 변수) 
		{
			처리할 내용들;
			...
		...
		-> 반복할 때마다 '배열 변수나 Collection 계열 변수'의 데이터를 하나씩 꺼내와 '변수명'에 저장한 후 '처리할 내용들'을 처리한다.
*/		
		...
		// 만약 제네릭을 사용하지 않은 Collection 을 향상된 for문으로 처리할 경우에는 Object형으로 처리한다.
...
//		System.out.println();	System.out.println();
		...
		v.ensureCapacity(5); // 현재 용량이 최소보다 작다면
							 // 신규용량 = 현재용량 * 2
