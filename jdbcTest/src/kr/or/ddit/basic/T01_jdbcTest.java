 ...
 *  -> JDBC 드라이버는 DB를 반든 회사에서 제공한다
 *  Class.forName("oracle.jdbc.driver.OracleDriver");
 *  
 *  2. 접속하기 : 접속이 성공하면 Connection 객체가 생성된다
 *  DriverManager.getConnection() 메서드를 이용한다
 *  
 *  3. 질의 : Statement 객체 또는 PreparedStatement객체를 이용하여 SQL 문장을 싱행한다.
 *  
 *  4. 결과 :
 *  	1) SQL문이 SELECT일 경우 -> ResultSet 객체가 만들어진다. ResultSet 객체에는 SELECT한 결과가 저장된다
 *  	2) SQL문이 INSERT, UPDATE, DELETE일 경우 -> 정수값을 반환한다 (정수값은 보통 실행에 성공한 레코드 수를 말한다)
 */
